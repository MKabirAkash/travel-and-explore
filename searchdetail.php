...
if(isset($_GET['id']) && ctype_digit($_GET['id'])){
  ...
  $isFullList = false;
  $did = intval($_GET['id']);
  $strSQL = "SELECT * FROM destinations WHERE id=?";
  ...
  if($prepared->rowCount() == 1){
    ...
    $row = $prepared->fetch();
    $id=$_GET['id'];
    $query2 = "SELECT ROUND(AVG(rating),1) as averageRating FROM post_rating WHERE postid=?";
    $result2 =$conn->prepare($query2);
    $result2->execute(array($did));
    $rows=$result2->fetch();
    ...
    echo '
            ...
            <p>'. $row['state'] .'</p>
            <p>'. $row['infor'] .'</p>
            <p>'. $rows['averageRating'].'</p>
      </div>
      <div class="result2">
      <h3><a href="rate.php?id='.$did.'" class="text-dark">Rate this place</a> </h3>
      </div>'; 
